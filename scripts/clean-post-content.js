...
function cleanHtmlContent(content) {
    ...
    cleaned = cleaned.replace(/<span[^>]*class="[^"]*glyphicon-user[^"]*"[^>]*>[\s\S]*?<\/span>/gi, '');
    cleaned = cleaned.replace(/<span[^>]*class="[^"]*glyphicon-time[^"]*"[^>]*>[\s\S]*?<\/span>/gi, '');
    
    cleaned = cleaned.replace(/Victor Oliveira[\s\S]*?(?=<p>|<h[1-6]|<ul>|<ol>|<table>|<pre>|<blockquote>)/gi, '');
    ...
    cleaned = cleaned.replace(/<span[^>]*class="[^"]*glyphicon-folder-open[^"]*"[^>]*>[\s\S]*?<\/span>/gi, '');
    cleaned = cleaned.replace(/<span[^>]*class="[^"]*glyphicon-bookmark[^"]*"[^>]*>[\s\S]*?<\/span>/gi, '');
    
    cleaned = cleaned.replace(/<a[^>]*class="[^"]*tag_cloud[^"]*"[^>]*>[\s\S]*?<\/a>/gi, '');
    ...
    cleaned = cleaned.replace(/<\/div>\s*<\/p>/gi, '</p>');
