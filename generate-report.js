...
const reportData = JSON.parse(fs.readFileSync('test-report.json', 'utf-8'));
...
<head>
    ...
    <style>
        ...
        .test-case.passed-row { background: #f8fff9; }
        .test-case.failed-row { background: #fff8f8; }
        .status { padding: 3px 10px; border-radius: 3px; font-size: 0.9em; font-weight: bold; }
        .status.passed-status { background: #28a745; color: white; }
        .status.failed-status { background: #dc3545; color: white; }
    ...
    <div class="container">
        ...
        <div class="summary">
            <div class="summary-box passed">
                <h2>${reportData.numTotalTests - reportData.numFailedTests}</h2>
                <p>Tests Passed</p>
            </div>
            <div class="summary-box failed">
                <h2>${reportData.numFailedTests}</h2>
                <p>Tests Failed</p>
            </div>
            <div class="summary-box passed">
                <h2>${reportData.numTotalTestSuites}</h2>
                <p>Test Suites</p>
            </div>
        </div>`;
...
reportData.testResults.forEach(suite => {
    ...
    suite.testResults.forEach(test => {
        const rowClass = test.status === 'passed' ? 'passed-row' : 'failed-row';
        const statusClass = test.status === 'passed' ? 'passed-status' : 'failed-status';
        html += `<div class="test-case ${rowClass}">
